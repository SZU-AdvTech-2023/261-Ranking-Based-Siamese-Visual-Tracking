...
# 下面为标签平滑的损失函数
def get_label_smoothing_loss(pred, label, select, smoothing=0.1):
    if len(select.size()) == 0 or \
            select.size() == torch.Size([0]):
        return 0
    pred = torch.index_select(pred, 0, select)
    label = torch.index_select(label, 0, select)
    if label[0] == 0:
        unlabel = torch.ones_like(label)
    else:
        unlabel = torch.zeros_like(label)
    loss_confident = F.nll_loss(pred, label)
    loss_unconfident = F.nll_loss(pred, unlabel)
    return loss_confident * (1-smoothing) + loss_unconfident * smoothing
...
def select_cross_entropy_loss(pred, label):
    ...
    # 测试标签平滑
    # get_label_smoothing_loss(pred, label, pos)
    loss_pos = get_cls_loss(pred, label, pos)
    loss_neg = get_cls_loss(pred, label, neg)
    return loss_pos * 0.5 + loss_neg * 0.5

def label_smoothing_entropy_loss(pred, label):
    pred = pred.view(-1, 2)
    label = label.view(-1)
    pos = label.data.eq(1).nonzero().squeeze().cuda()
    neg = label.data.eq(0).nonzero().squeeze().cuda()
    ...
    # 标签平滑
    loss_pos = get_label_smoothing_loss(pred, label, pos)
    loss_neg = get_label_smoothing_loss(pred, label, neg)
    return loss_pos * 0.5 + loss_neg * 0.5
