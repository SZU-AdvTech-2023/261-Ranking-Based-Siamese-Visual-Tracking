...
class AdjustLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        SAW = torch.softmax(torch.matmul(q, k), dim=1)

        x0 = self.sample(x)
        B, C, H, W = x0.shape
        x0 = x0.contiguous().view(B, C, H * W)
        x0 = torch.matmul(x0, SAW).view(B, C, H, W)

        weightA = 0.5
        XS =torch.add(weightA * x0, x)

        x = self.downsample(XS)
